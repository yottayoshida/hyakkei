...
**Editing a golden sample (`packages/core/src/golden-fixtures/{applications,budget,regional}.json`):** these are canonical `dashboard.json` exemplars, not test-only data — they are the reference examples for issue #26's MCP contract, and their job here is to pin rendering: between them they cover all 7 chart types, so a renderer change shows up as a snapshot diff. Their data is invented to fit those chart types and is never published. Every sample is a real, load-bearing invariant enforced in CI, not just documentation: `golden-samples.roundtrip.test.ts` requires each one to pass authoring-schema validation with zero reference issues (dangling/duplicate/overlap/out-of-bounds/reserved-word), declare only `file`-kind sources, have every query's SQL `FROM` table match a declared source id, bake into a schema-valid `BakedDashboard` with zero baked-side reference issues, and produce non-empty rows for every configured chart. An edit that breaks any of these fails that test directly, with a message naming the sample and the exact issue — separately from (and usually without touching) the SVG/pixel golden snapshots above.

**Editing a gallery sample (`packages/core/src/gallery-samples/{population,economy,administration}.json`):** these are the dashboards published to GitHub Pages, built from fixed snapshots of official e-Stat tables (`docs/gallery-data.md`). They were split out of the golden fixtures in [ADR-0021](docs/adr/0021-gallery-samples-are-separate-from-golden-fixtures.md): a rendering pin wants whatever data makes each chart type render, while a public example wants whatever chart type the data actually supports, and sharing one set of files meant one of the two was always wrong. `gallery-samples.roundtrip.test.ts` runs the same round trip as above plus three guards that only apply to something a stranger will read — chart types restricted to an allowlist with the reason each excluded type is excluded, no national total on a categorical axis beside individual regions, and one consistent number of decimal places per table column.
