...
>
> - **Counts.** Context and the Alternatives table quote **7 of 22 guaranteed** and **3 known defects**. Fixing the chart color roles moved that principle from *defect* to *by construction*, giving **8 guaranteed** and **2 known defects**; on 2026-07-29 [#123](https://github.com/yottayoshida/hyakkei/issues/123) resolved the remaining two, so the current figures are **9 guaranteed** and **0 known defects** (the layout principle turned out to have been misread — the guidebook asks for on-grid placement, which the schema already makes unviolatable, not for a particular 16:9 division). `docs/guidebook-coverage.md` remains the single source; these lines are a snapshot of it on the day this ADR was written.
>
>   Concretely, **this note supersedes every count in the body below**: Context (§"…gives seven, and that is the figure worth quoting… defensibly 1, 3, or 7… Three confirmed conformance defects exist today") and the Alternatives row rejecting "full guidebook compliance" ("7 of 22 guaranteed… 3 known defects") both still read 7 and 3, and are left unedited on purpose — ADR bodies here record what was written on the day, and corrections go in a dated note rather than a rewrite (the same convention `docs/adr/0016-guideline-nudge-scope.md` states as "Original text below is unedited"). The 7 in the body was already stale before this PR: [#122](https://github.com/yottayoshida/hyakkei/issues/122) moved it to 8 on 2026-07-27 without editing those lines either. A reviewer reading the body alone would take the wrong figure, which is why it is spelled out here. Decision 5's *claim form* — "conforms to N of the 22 machine-checkable principles in guidebook version X" — is exactly what makes this survivable: the form did not change, only N. Two caveats travel with that zero and are recorded beside it in the coverage file: the single runtime predicate fires on a threshold hyakkei chose (now disclosed in the nudge's own citation), and `getGuidelineRules()` fails open — so on the v1.0 agent path, "0 nudges" must be distinguished from "could not evaluate" before it is reported as conformance ([ADR-0016](./0016-guideline-nudge-scope.md)'s own read-forward note).
> - **Decision 7 named four fields but not where any of them lives; three have now landed and one of the four moved.** On 2026-08-02, `meta.updatedAt` / `meta.sourceNote` / `meta.summary` shipped as optional `BaseMeta` fields drawn by a new dashboard footer ([#124](https://github.com/yottayoshida/hyakkei/issues/124), [ADR-0019](./0019-guidebook-do-side-fields-and-dashboard-chrome.md)). Two placements differ from what Decision 7's prose implies. `sourceNote` is **per dashboard, not per source** — `BakedDashboard` forbids `sources`, so a per-source field cannot reach the artifact a third party opens, which is the requirement's only audience. And `altText`, still owed, will land on `Chart` rather than on `ChartOptions`: that object's unknown-key **rejection** is pinned, so a document using it would fail to open in an older hyakkei. The count in Decision 5's claim form is unaffected — none of these three is `active` or `by construction`, and the coverage file records them under a new `supported` status precisely so they move no headline figure.
