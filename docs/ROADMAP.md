...
- **Guidebook conformance stated by count**, never as "full compliance": *"conforms to N of the M machine-checkable principles in guidebook version X; K are not machine-checkable; J are not yet covered."* Newly baked artifacts stamp the publisher label from `GUIDEBOOK_SOURCE` into optional `BakedMeta.guidebookVersion` and render it in the footer; the label is provenance, not a content hash. [docs/guidebook-coverage.md](./guidebook-coverage.md) is the canonical inventory
- **Schema extension is limited to Do-side principles** the guidebook requires and the schema cannot express: `altText`, summary text, `updatedAt`, `sourceNote` — four additive fields, now all landed (the three metadata fields on 2026-08-02 and `Chart.altText` on 2026-08-06; [ADR-0019](adr/0019-guidebook-do-side-fields-and-dashboard-chrome.md)). Rules whose *violations* the schema cannot express stay `doc-only` and are documented as enforced by construction — see ADR-0017 Decision 7
