...
> **Partly absorbed into v1.0 (2026-07-27, ADR-0017)**, and **now measurable for the first time**. A full inventory of the guidebook (PDF v02, all 59 pages, cross-checked against the official 27-item checklist) found **22 machine-checkable principles**. **Seven are guaranteed in practice** (rule-enforced or impossible to violate); of those, three come from a named rule and one has a runtime predicate. So this outcome sits somewhere in 1–7 of 22 depending on which question is asked — a range, but a knowable one, where before it was an unknown fraction. `docs/guidebook-coverage.md` says which reading applies where. Four of the remaining principles need a new schema field before they can be satisfied at all ([#124](https://github.com/yottayoshida/hyakkei/issues/124)); that subset is v1.0's schema work. The rest, plus maps and the remaining chart types, stay here. `docs/guidebook-coverage.md` is the canonical inventory. **Three conformance defects were found during the same inventory** ([#122](https://github.com/yottayoshida/hyakkei/issues/122), [#123](https://github.com/yottayoshida/hyakkei/issues/123)) — they are hyakkei's own bugs and independent of any version's scope.
>
...
- **Schema extension is limited to Do-side principles** the guidebook requires and the schema cannot express: `altText`, summary text, `updatedAt`, `sourceNote` — four additive fields. Rules whose *violations* the schema cannot express stay `doc-only` and are documented as enforced by construction — see ADR-0017 Decision 7
...
**Known constraint at time of writing**: MCP Apps does not render in Claude Desktop, claude.ai, or Claude Code for Web ([`ext-apps#671`](https://github.com/modelcontextprotocol/ext-apps/issues/671), open since 2026-05-27). It is host-side and unfixable from here — the same servers render in Cowork. v1.0 may therefore ship with the App implemented and conformance-verified but the preview dark. That determines whether a feature is lit on release day, not whether the shape is right. See `docs/spikes/mcp-transport-gate.md`.
...
- `dashboard.json` schema: additive-only within a major version. **A breaking schema change is what would force a new schema major version** — this is about `"version": 1` in the document, not about the product's version number. (Before 2026-07-27 this line read "…is what makes a 2.0," which now collides with the v2.0 section above: that renumbering was a scope move, not a schema break. The schema is still at v1 and additive-only.) v1.0's Do-side additions — `altText`, summary text, `updatedAt`, `sourceNote` — are all additive and keep `"version": 1`. The one guidebook item that would *not* be additive (a vertical grid constraint, narrowing `LayoutItem.y`/`h`) is deliberately excluded from v1.0 for that reason
