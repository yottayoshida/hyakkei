...
- [x] Chart library: **done 2026-07-10** (issue #4, `docs/spikes/m0-charts.md`). ECharts 6.1.0 SVG renderer confirmed: byte-exact hex fidelity across all 7 key colors × 2 appearances, functional ARIA/decal, acceptable performance. Palette semantics confirmed as 7 key-color templates (Solid Gray/Blue/Light Blue/Cyan/Green/Orange/Red), each a 6-step ramp + shared Yellow accent + Semantic Success/Error — neither the old "7-color categorical" nor "single-hue ramp + red accent" framing was correct (PRD F6, corrected). ADR-0004's chart-library row is now Accepted; the Vega-Lite escape hatch did not trigger.
