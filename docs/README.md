...
- **[docs/guidebook-coverage.md](./guidebook-coverage.md) is new** and is the denominator for every conformance claim: 22 machine-checkable principles in the guidebook, **9** guaranteed in practice, of which 3 come from named rules and 1 from a runtime predicate; **0** known defects. (Was 7 / 3, then 8 / 2 on 2026-07-27 when [#122](https://github.com/yottayoshida/hyakkei/issues/122) moved row 8 from defect to by-construction — see [ADR-0018](./adr/0018-chart-color-roles-follow-the-guidebook-role-layer.md) — and 9 / 0 on 2026-07-29 via [#123](https://github.com/yottayoshida/hyakkei/issues/123).) The file states which count answers which question — the same implementation is defensibly 1, 3, or 9. Conformance is stated by count — never as "full compliance."
- **Zero known defects does not mean nothing is left.** Read the two qualifiers the coverage file attaches to that zero: the single runtime rule fires on a threshold hyakkei chose rather than one the guidebook states (now disclosed to the user in the nudge's own citation), and the rule engine fails open, so a path that reports "0 nudges" as a guarantee would report it after failing to evaluate ([ADR-0016](./adr/0016-guideline-nudge-scope.md)). Neither is a defect against a guidebook principle, which is the only thing that row counts.
